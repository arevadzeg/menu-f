...
const Carousel: React.FC = () => {
    ...
    const { appName, categoryId } = useParams<{ appName: string; categoryId?: string }>();
    const router = useRouter();
    const { data: slides, isLoading } = useGetCategories();


    const handleNavigateToCategory = useCallback((id: string) => {
        if (!appName) return;
    ...
    return (
        <section id="carousel" className='carousel'>
            <button className="carousel__nav carousel__nav--left" onClick={scrollPrev}>
                <ChevronLeftIcon />
            </button>
            ...
            <button className="carousel__nav carousel__nav--right" onClick={scrollNext}>
                <ChevronRightIcon />
            </button>
        </section>
    );
