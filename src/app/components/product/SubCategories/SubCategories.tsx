...
const SubCategories = () => {
  const { categoryId, appName, subCategoryId } = useParams(); // Extract category ID from URL
  ...
  const handleNavigateToSubCategory = (subCategoryId: string) => {
    setSelectedSubCategory(subCategoryId); // Set selected category ID
      ...
      {category ? (
        <>
          ...
          <ul>
            {isAdmin && (
              <RadixButton
                className={`cursor-pointer  rounded-md transition-colors ${"bg-gray-100 hover:bg-blue-200"}`}
              >
                + Add Sub Category
              </RadixButton>
            )}
            {category.subCategories.map((sub) => (
              <li
                ...
                className={`sub-categorie ${
                  selectedSubCategory === sub.id ? "sub-categorie-selected" : ""
                }`}
                onClick={() => handleNavigateToSubCategory(sub.id)} // Set selected subcategory on click
              >
                {sub.name}

                {isAdmin && <DotsHorizontalIcon className="cursor-pointer" />}
              </li>
            ))}
          </ul>
        </>
